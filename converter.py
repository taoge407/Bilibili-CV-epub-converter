        ...
        try:
            ...
            if content_div:
                ...
                for img in content_div.find_all("img"):
                    ...
                    img['src'] = "images/image_" + str(src.split("/")[-1].split("@")[0])
...
class Converter:
    ...
    def convert_epub(self):
        ...
        ebook.set_cover("cover.jpg", download_image(readlist.cover_url))
        ...
        for image_url in image_urls_set:
            try:
                ...
                print(e)
        ebook.toc = [(epub.Section(readlist.name), chapters)]
        ebook.add_item(epub.EpubNcx())
        ebook.add_item(epub.EpubNav())
        ebook.spine = ["nav"] + chapters

        # 写入文件
        epub.write_epub(f"{readlist.name}.epub", ebook, {})

def download_image(url) -> bytes | None | Any:
    img_id = url.split("/")[-1].split(".")[0]
    os.makedirs("./.cache/imgs", exist_ok=True)
    fn = f"./.cache/imgs/{img_id}"
